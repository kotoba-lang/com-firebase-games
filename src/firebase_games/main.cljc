(ns firebase_games.main
  "Kotodama WASM entrypoint for the Firebase_games clean-room actor (L4) — Clojure port.
  ...
  facts (`firebase_games.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "firebase_games")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"firebase_games.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "firebase_games-compat" :tier tier :entities entities} 200])
...
(defn start! [] :firebase_games-compat/ready)
